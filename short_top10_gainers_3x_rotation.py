...
CHASE_MARGIN = 100.0      # 追涨策略单币保证金
CHASE_LEVERAGE = 3.0      # 追涨杠杆
CHASE_HOLD_HOURS = 11     # 持仓时间
...
def load_state():
    ...
    if CHASE_STRAT_ID not in data:
        data[CHASE_STRAT_ID] = {
            "balance": 1000.0, "positions": [], "total_invested": 1000.0, "liquidation_count": 0, "prev_top10": []
        }
    # 确保 prev_top10 字段存在
...
def run_chase_strategy(data, market_map, top_10):
    """
    逻辑：对比上一次和这一次Top10，找出新上榜的币开多。持仓满11小时平仓。
    """
    ...
    remaining = []
    for pos in strat['positions']:
        hold_time = (current_ts - pos['entry_time']) / 3600.0
        if hold_time >= CHASE_HOLD_HOURS:
            ...
            pnl = (curr - entry) * amt # 做多盈亏
            strat['balance'] += pnl
            
            note = f"追涨平仓({hold_time:.1f}h)"
    ...
    if not prev_top10:
        ...
        for sym in new_coins:
            ...
            new_pos = {
                "symbol": sym, "entry_price": price, "margin": margin, "amount": amt,
            ...
            log_to_csv("OPEN_LONG", CHASE_STRAT_ID, sym, price, price, amt, 0, 
                       strat['balance'], strat['total_invested'], margin, 0, chg, "新币追涨")
            print(f"   >> 发现新币 {sym}，开多！")
            acted = True
